...
lights = ImageSequence(os.path.join('data', '2022-01-11_jwst', '2022-01-11-0941_0-CapObj.SER'), frame_step = 10)
darks = ImageSequence(os.path.join('data', '2022-01-11_jwst', '2022-01-11-1027_1-CapObj.SER'), frame_step = 10)
...
def read_average_image_with_alignment_transforms(lights, alignment_transforms, losses, image_shape, dark_image):
...
def align_images(lights, dark_image):
    ...
    losses = tf.Variable(tf.ones(len(lights)))
    ...
    for lod in range(num_lods - 1, skip_lods - 1, -1):
        ...
        # with some cool losses thing?
        #target_image = read_average_image_with_alignment_transforms(lights, alignment_transforms, losses, image_shape, dark_image)
        # with just equal weights...
        #target_image = read_average_image_with_alignment_transforms(lights, alignment_transforms, tf.ones_like(losses), image_shape, dark_image)
        # eh just get the middle one...
        target_image = lights[int(len(lights) / 2)]
        ...
        debug = True
        if debug:
            equal_weight_image = read_average_image_with_alignment_transforms(lights, alignment_transforms, tf.ones_like(losses), image_shape, dark_image)
        ...
        for pass_index in range(passes_per_lod):
            ...
            for image_index, image_bhwc in enumerate(lights):
                ...
                # cener everything
                alignment_transforms.assign(alignment_transforms - tf.reduce_mean(alignment_transforms, axis = 1, keepdims=True))
...
alignment_transforms = align_images(lights, dark_image)
...
for image_index, image_bhwc in lights:
    if dark_image is not None:
